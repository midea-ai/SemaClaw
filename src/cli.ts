...
// 与 daemon 共享同一份 model.conf。否则子命令走 sema-core 默认的
// ~/.sema/model.conf，跟 web UI 的模型切换完全脱节。
//
// 路径由 src/config.ts 解析：
//   细粒度 env SEMACLAW_MODEL_CONF_PATH > ${SEMACLAW_CONFIG_HOME}/semaclaw-model.conf
{
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const { config } = require('./config') as typeof import('./config');
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const { setModelConfigPathOverride } = require('sema-core') as { setModelConfigPathOverride: (p: string) => void };
  setModelConfigPathOverride(config.paths.modelConfPath);
}
