  ...
  'schedule_task',
  {
    ...
    inputSchema: {
      ...
      script_command: z.string().optional().describe(
        'Bash command to execute (required for "script" and "script-agent" modes). The command is run via bash shell, so any interpreter is supported — e.g. "python3 /path/to/check.py", "node /app/report.js", "bash /home/user/fetch.sh". No path restrictions apply; the command runs with the same permissions as the semaclaw process.'
      ),
    },
  },
